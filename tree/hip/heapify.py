...
def heapify(tree, index, tree_size):
    ...
    # 일단 부모 노드의 값이 가장 크다고 설정
    largest = index

    # 왼쪽 자식 노드가 존재하는지 확인하고 부모 노드의 값보다 큰지 확인
    ...
    if largest != index:  # 부모 노드의 값이 자식 노드의 값보다 작으면
        swap(tree, index, largest)  # 부모 노드와 최댓값을 가진 자식 노드의 위치 교환
        heapify(tree, largest, tree_size)  # 자리가 바뀌어 자식 노드가 된 기존의 부모 노드를 대상으로 또 heapify
